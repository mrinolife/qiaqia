/* YaHa 呀哈 — HSK1 + Taiwan-survival Chinese, chiikawa-style.
...
function blankState() {
  return { doneLessons: {}, srs: {}, xp: 0, streak: { last: "", count: 0 }, tripDate: DEFAULT_TRIP,
           ...
           activeDays: {}, daily: {}, name: "Rachel", wrong: {}, speakingOn: true, theme: "chiikawa" };
}
const S = loadState();
...
/* streak milestone toasts — big named days get their own line, everything
   else in between still gets a bit of rotating flavor instead of silence */
const STREAK_MILESTONES = {
  3:  ["🔥 3 days in a row — a real habit forming!", "3-day streak! 呀哈, you're on fire!"],
  5:  ["🔥 5 days straight — 加油, keep the fire lit!"],
  7:  ["🔥🔥 one full week! 一周了! amazing consistency!", "7 days straight — a whole week of effort!"],
  14: ["🔥 two weeks straight! 两个星期了! unstoppable!"],
  21: ["🔥 21 days — they say that's how habits stick!"],
  30: ["🔥🔥🔥 30 days!! 一个月了! this is basically a lifestyle now!"],
  50: ["🔥 50-day streak! legendary dedication!"],
  100: ["🔥💯 100 DAYS!! 一百天!! certified 討伐-class hunter energy!"],
};
const STREAK_DAILY_LINES = [
  n => `🔥 ${n} days and counting!`,
  n => `🔥 streak alive — day ${n}!`,
  n => `🔥 day ${n}! 加油 keeps working!`,
  n => `🔥 ${n} in a row — nice and steady!`,
];
function streakToast(n) {
  if (n <= 1) return;
  const milestone = STREAK_MILESTONES[n];
  if (milestone) { toast(shuffle(milestone)[0]); confetti(); return; }
  toast(shuffle(STREAK_DAILY_LINES)[0](n));
}
function bumpStreak() {
  const t = today();
...
const DAY_GOAL = 20;
/* level-up + daily-goal toasts rotate through a few lines each so the same
   fixed string doesn't repeat every single time she crosses a threshold */
const LEVELUP_LINES = [
  li => `呀哈!! LEVEL UP!! LV${li.lv} · ${li.title}`,
  li => `叮!! new rank unlocked — LV${li.lv} ${li.title}!`,
  li => `🎉 LV${li.lv}!! ${li.title} — leveled up!`,
...
const DAY_GOAL_LINES = [
  ...
  "🌟 that's today's XP goal! 加油完成!",
  "🌟 goal met for today — 做到了!",
];
function addXP(n) {
  const before = levelInfo(S.xp).lv;
  const dayBefore = (S.dayXP || {})[today()] || 0;
  S.xp += n;
  S.dayXP = S.dayXP || {};
...
/* ================= data ================= */
const D = window.YAHA_DATA || { vocab: [], phrases: [], dialogues: [], tones: [] };
const T = window.YAHA_TAIWAN || { grammar: [], scenarios: [], dialogues: [], culture: [] };
const CHATS = window.YAHA_CHATS || [];
...
/* Doraemon-world name map — id -> Doraemon character display name, swapped
   in via activeMascotNames() whenever S.theme==="doraemon". */
const MASCOT_NAMES_DORAEMON = { chiikawa: "のび太 Nobita", hachiware: "ドラえもん Doraemon", usagi: "ジャイアン Gian",
                       momonga: "スネ夫 Suneo", kurimanju: "セワシ Sewashi",
                       rakko: "出木杉 Dekisugi", shisa: "ドラミ Dorami",
                       kani: "ジャイ子 Jaiko", yoroi: "しずか Shizuka", chimera: "ミニドラ Mini-Doraemon" };
function activeMascotNames() { return S.theme === "doraemon" ? MASCOT_NAMES_DORAEMON : MASCOT_NAMES; }
...
const CAST_SPEAK = {
  chiikawa:  ["今天也加油!", "小小的努力也是努力!", "呜呜…我们一起学吧", "可以的…我觉得可以的!", "呜, 好难…但是不放弃!", "做到了! 太好了…"],
  hachiware: ["太好了!", "没关系, 慢慢来~", "能学会的!", "深呼吸, 一起来!", "错了也没关系哦~", "你今天也很棒!"],
  ...
  shisa:     ["嘿嘿! 今天也练习吧!", "台湾见!", "台湾的夜市很热闹哦!", "路上小心, 一路平安!", "学好这句, 台湾用得上!", "嘿嘿, 离台湾又近一步了!"],
};
/* Doraemon-world dialogue — same ids/thresholds/shape as CAST_SPEAK above,
   index-paired 1:1 with CAST_DORAEMON[].lines below (same order/length per
   character), swapped in via activeCastSpeak() whenever S.theme==="doraemon". */
const CAST_SPEAK_DORAEMON = {
  chiikawa:  ["今天也加油!", "呜呜…好难, 但是我会努力!", "我可以的…我觉得可以的!", "一起学吧, 静香!", "呜, 又错了…不放弃!", "做到了! 太好了…"],
  hachiware: ["没问题! 交给我吧!", "别担心, 慢慢来~", "深呼吸, 一起想办法!", "你能学会的!", "错了也没关系哦~", "太好了! 你今天也很棒!"],
  usagi:     ["嘿呀——!! 上课啦!!", "台湾!! 珍珠奶茶!! 冲啊!!", "嘿呀!! 学完了!!", "好耶好耶好耶!!", "哇哈哈哈!! 我最厉害!!", "冲冲冲!!"],
  momonga:   ["看我的! 我家超厉害的!", "哼哼, 我早就会了!", "我是不是很厉害!!", "再夸我一次!! 嘿嘿", "看! 我的分数! 快看!!", "全世界都该看到这个!!"],
  kani:      ["加油加油! 我陪你!", "好耶! 干得漂亮!", "学完一课啦, 咔嚓!", "偶像都在为你加油哦!", "好耶好耶! 太棒了!", "下一课, 出发!"],
  kurimanju: ["嗯…慢慢来, 急什么呢", "我是从未来来的哦~ 慢慢学", "唔…不急, 一步一步来", "学完了? 那就休息一下吧~", "慢工出细活儿, 慢慢学~", "嗯, 你很努力, 我看在眼里"],
  chimera:   ["一起玩吧~ 呼呼!", "嘿嘿, 你进步好快!", "咦? 猜猜我是谁~", "飞呀飞~ 一起去下一关!", "嘿嘿嘿, 我们人很多哦~", "你好厉害, 我都想鼓掌!"],
  rakko:     ["不错。 继续。", "轻轻松松。", "继续。", "还行。 下一个。", "别停。 你在进步。", "台湾等着你呢。"],
  yoroi:     ["你很努力, 真棒!", "辛苦了! 做得很好!", "每一步都算数, 别灰心!", "你已经做得很好了!", "慢慢来, 我陪着你!", "今天的你, 也很了不起!"],
  shisa:     ["嘿! 今天也练习吧!", "台湾见! 我来带路!", "台湾的夜市很热闹哦!", "路上小心, 一路平安!", "学好这句, 台湾用得上!", "别怕, 有我在!"],
};
...
const CAST = [
  { id: "chiikawa",  unlock: 0,    blurb: "small, brave, tries so hard 🥺", lines: ["今天也加油! (jiāyóu — you got this!)", "小小的努力也是努力! tiny effort still counts!", "呜呜…我们一起学吧 — let's learn together!", "可以的…我觉得可以的! (kěyǐ de — I think we can do it!)", "呜, 好难…但是不放弃! (bù fàngqì — but I won't give up!)", "做到了! 太好了… (zuòdào le — we did it! so glad…)"] },
  { id: "hachiware", unlock: 0,    blurb: "the optimist — 'we can do it!'", lines: ["太好了! ready for a tiny lesson?", "没关系, 慢慢来~ no rush, we'll get there!", "能学会的! we can totally do this!", "深呼吸, 一起来! (shēnhūxī — deep breath, let's go!)", "错了也没关系哦~ (cuò le yě méiguānxi — mistakes are fine too!)", "你今天也很棒! (nǐ jīntiān yě hěn bàng — you're great today too!)"] },
  ...
  { id: "shisa",     unlock: 1100, blurb: "guardian of the trip! 一路平安", lines: ["嘿嘿! 今天也练习吧!", "台湾见! see you in Taiwan!", "台湾的夜市很热闹哦! (yèshì hěn rènao — the night markets are so lively!)", "路上小心, 一路平安! (yílù píng'ān — safe travels, stay safe!)", "学好这句, 台湾用得上! (yòng de shàng — you'll actually use this in Taiwan!)", "嘿嘿, 离台湾又近一步了! (yòu jìn yí bù le — one step closer to Taiwan!)"] },
];
/* Doraemon-world cast roster — same ids/unlock thresholds as CAST above,
   index-paired 1:1 with CAST_SPEAK_DORAEMON (same order/length per character). */
const CAST_DORAEMON = [
  { id: "chiikawa",  unlock: 0,    blurb: "earnest, clumsy, tries so hard 🥺", lines: ["今天也加油! (jiāyóu — you got this today too!)", "呜呜…好难, 但是我会努力! (kùnnán — it's hard, but I'll try!)", "我可以的…我觉得可以的! (wǒ kěyǐ de — I think I can do it!)", "一起学吧, 静香! (yìqǐ xué ba — let's study together!)", "呜, 又错了…不放弃! (bù fàngqì — got it wrong again… but not giving up!)", "做到了! 太好了… (zuòdào le — I did it! so glad…)"] },
  { id: "hachiware", unlock: 0,    blurb: "the reassuring helper — loves dorayaki", lines: ["没问题! 交给我吧! (jiāo gěi wǒ ba — no problem, leave it to me!)", "别担心, 慢慢来~ (bié dānxīn — don't worry, take your time~)", "深呼吸, 一起想办法! (yìqǐ xiǎng bànfǎ — deep breath, let's figure it out together!)", "你能学会的! (nǐ néng xuéhuì de — you can learn this!)", "错了也没关系哦~ (cuò le yě méiguānxi — it's okay to get it wrong too~)", "太好了! 你今天也很棒! (nǐ jīntiān yě hěn bàng — great, you're awesome today too!)"] },
  { id: "usagi",     unlock: 0,    blurb: "loud, brash, big-voiced chaos energy", lines: ["嘿呀——!! 上课啦!! (shàngkè la — class time!!)", "台湾!! 珍珠奶茶!! 冲啊!! (chōng a — Taiwan!! bubble tea!! CHARGE!!)", "嘿呀!! 学完了!! (xué wán le — done studying!!)", "好耶好耶好耶!! (hǎo yē — yesss yesss yesss!!)", "哇哈哈哈!! 我最厉害!! (wǒ zuì lìhài — I'm the strongest!!)", "冲冲冲!! (chōng — GO GO GO!!)"] },
  { id: "momonga",   unlock: 100,  blurb: "boastful showoff, loves bragging", lines: ["看我的! 我家超厉害的! (wǒ jiā chāo lìhài de — check this out! my family's amazing!)", "哼哼, 我早就会了! (wǒ zǎo jiù huì le — hmph, I already knew this!)", "我是不是很厉害!! (wǒ shì bú shì hěn lìhài — aren't I amazing!)", "再夸我一次!! 嘿嘿 (zài kuā wǒ yí cì — praise me once more!! heh heh)", "看! 我的分数! 快看!! (kuài kàn — look, my score, quick, look!!)", "全世界都该看到这个!! (quán shìjiè — the whole world should see this!!)"] },
  { id: "kani",      unlock: 220,  blurb: "cheery, blunt, idol-obsessed", lines: ["加油加油! 我陪你! (wǒ péi nǐ — go go go, I'm with you!)", "好耶! 干得漂亮! (gàn de piàoliang — yay! nicely done!)", "学完一课啦, 咔嚓! (kāchā — snip, one lesson done!)", "偶像都在为你加油哦! (ǒuxiàng — even the idols are cheering for you!)", "好耶好耶! 太棒了! (tài bàng le — yay yay, so great!)", "下一课, 出发! (chūfā — next lesson, let's launch!)"] },
  { id: "kurimanju", unlock: 360,  blurb: "calm, wise-beyond-years, sleepy elder", lines: ["嗯…慢慢来, 急什么呢 (bié zháojí — hm, take your time, no need to rush)", "我是从未来来的哦~ 慢慢学 (wèilái — I'm from the future, you know~ study slowly)", "唔…不急, 一步一步来 (yí bù yí bù lái — no rush, one step at a time)", "学完了? 那就休息一下吧~ (xiūxi yíxià — done studying? then rest a little~)", "慢工出细活儿, 慢慢学~ (màn gōng chū xìhuór — good things take time~)", "嗯, 你很努力, 我看在眼里 (wǒ kàn zài yǎnlǐ — you're trying hard, I see it)"] },
  { id: "chimera",   unlock: 520,  blurb: "small, mysterious, playful mini swarm", lines: ["一起玩吧~ 呼呼! (yìqǐ wán ba — let's play together~ whee!)", "嘿嘿, 你进步好快! (jìnbù hǎo kuài — heh heh, you're improving so fast!)", "咦? 猜猜我是谁~ (cāicāi wǒ shì shéi — huh? guess who I am~)", "飞呀飞~ 一起去下一关! (fēi ya fēi — flying off to the next round!)", "嘿嘿嘿, 我们人很多哦~ (wǒmen rén hěn duō — heh heh, there's a lot of us~)", "你好厉害, 我都想鼓掌! (gǔzhǎng — you're so good, makes me want to clap!)"] },
  { id: "rakko",     unlock: 700,  blurb: "cool, effortlessly excellent, understated", lines: ["不错。 继续。 (búcuò — not bad. keep going.)", "轻轻松松。 (qīngqīngsōngsōng — easy, no big deal.)", "继续。 (jìxù — continue.)", "还行。 下一个。 (hái xíng — not bad. next.)", "别停。 你在进步。 (bié tíng — don't stop. you're improving.)", "台湾等着你呢。 (Táiwān děngzhe nǐ ne — Taiwan's waiting for you.)"] },
  { id: "yoroi",     unlock: 900,  blurb: "kindest, most encouraging, gentle warmth", lines: ["你很努力, 真棒! (nǐ hěn nǔlì — you're so diligent, wonderful!)", "辛苦了! 做得很好! (xīnkǔ le — good work, well done!)", "每一步都算数, 别灰心! (měi yí bù dōu suànshù — every step counts, don't lose heart!)", "你已经做得很好了! (yǐjīng zuò de hěn hǎo le — you're already doing so well!)", "慢慢来, 我陪着你! (wǒ péizhe nǐ — take your time, I'm with you!)", "今天的你, 也很了不起! (hěn liǎobùqǐ — today's you is amazing too!)"] },
  { id: "shisa",     unlock: 1100, blurb: "capable guide, guardian of the trip", lines: ["嘿! 今天也练习吧! (jīntiān yě liànxí ba — hey! let's practice today too!)", "台湾见! 我来带路! (wǒ lái dàilù — see you in Taiwan! I'll lead the way!)", "台湾的夜市很热闹哦! (yèshì hěn rènao — the night markets are so lively!)", "路上小心, 一路平安! (yílù píng'ān — safe travels, stay safe!)", "学好这句, 台湾用得上! (yòng de shàng — you'll actually use this in Taiwan!)", "别怕, 有我在! (yǒu wǒ zài — don't be afraid, I've got you!)"] },
];
function activeCast() { return S.theme === "doraemon" ? CAST_DORAEMON : CAST; }
function activeCastSpeak() { return S.theme === "doraemon" ? CAST_SPEAK_DORAEMON : CAST_SPEAK; }
function unlockedCast() { return activeCast().filter(c => S.xp >= c.unlock); }
...
const SNACKS = (window.YAHA_FOODS && window.YAHA_FOODS.length >= 12) ? window.YAHA_FOODS : SNACKS_FALLBACK;
function awardSnack() {
  S.snacks = S.snacks || {};
...
function checkUnlocks() {
  ...
  if (fresh.length) {
    ...
    const ov = el(`<div class="unlock-pop"><div class="card pink bigcard">
        ...
        <div><b>${esc(activeMascotNames()[c.id])}</b></div>
...
/* Doraemon-world theme: maps each chiikawa-cast id -> the matching Doraemon-cast
   art file id (chars/doraemon_*.png, sourced separately, real dialogue content
   is phase 2 — this pass only wires which image shows when S.theme === "doraemon"). */
const THEME_ART_DORAEMON = {
  hachiware: "doraemon_hachiware", chiikawa: "doraemon_nobita", usagi: "doraemon_gian",
  momonga: "doraemon_suneo", yoroi: "doraemon_shizuka", rakko: "doraemon_dekisugi",
  shisa: "doraemon_dorami", kani: "doraemon_jaiko", kurimanju: "doraemon_sewashi",
  chimera: "doraemon_mini",
};
...
function mascotSVG(kind, size) {
  const s = size || 96;
  const themeKind = (S.theme === "doraemon" && THEME_ART_DORAEMON[kind]) ? THEME_ART_DORAEMON[kind] : kind;
...
/* ================= theme (chiikawa / doraemon world) =================
   chiikawa is the default: dataset.theme stays empty/absent for it, so
   [data-theme="doraemon"] CSS only ever matches the non-default state. */
function applyTheme() {
  document.documentElement.dataset.theme = S.theme === "doraemon" ? "doraemon" : "";
  const btn = document.getElementById("themeToggle");
  if (btn) {
    const isDoraemon = S.theme === "doraemon";
    btn.textContent = isDoraemon ? "🔔" : "🐱";
  ...
  // (already set above) on its first render. Later calls (theme toggle) are
  // always post-load, so the redraw fires normally.
...
if (themeToggleBtn) themeToggleBtn.onclick = () => { S.theme = S.theme === "doraemon" ? "chiikawa" : "doraemon"; save(); applyTheme(); };
applyTheme();
...
function maybePeek() {
  ...
  // usagi is the favorite — she's the app's namesake, so she peeks in most of the time
  const c = Math.random() < 0.62 ? activeCast().find(x => x.id === "usagi") : shuffle(unlockedCast())[0];
  ...
  pk.onclick = () => {
    toast(line);
    const spoken = (activeCastSpeak()[c.id] || [])[idx];
...
const TRAD = window.YAHA_TRAD || {}, TRAD_CHAR = window.YAHA_TRAD_CHAR || {};
function tradOf(s) {
  if (!s) return null;
...
function wordPopup(w) {
  ...
  ov.querySelectorAll("#wpChars .tile").forEach(t => t.onclick = () => {
    ...
    if (window.HanziWriter && window.YAHA_STROKES && window.YAHA_STROKES[c]) {
      HanziWriter.create(box, c, { width: 166, height: 166, padding: 8, showOutline: true,
        strokeColor: "#4a3b30", outlineColor: "#eadfd2",
        charDataLoader: (ch, done) => done(window.YAHA_STROKES[ch]) }).animateCharacter();
...
function renderFriendsInto(view) {
  ...
  activeCast().forEach(c => {
    ...
    const cell = el(`<button class="card friendcell ${got ? "" : "locked"}">
        ...
        <div class="fname">${got ? esc(activeMascotNames()[c.id]) : "???"}</div>
...
function voiceCheck() {
  ...
  view.append(el(`<div class="card ${zh.length ? "mint" : "yellow"}">
      ...
      <div class="cardrow" style="justify-content:flex-start">
        <button class="btn small blue" id="tts1">🔊 你好! 我是呀哈</button>
  ...
  document.getElementById("tts1").onclick = () => { document.getElementById("ttsState").textContent = zh.length ? "playing… (if silent, check media volume)" : "trying anyway — may be silent without a Chinese voice"; speak("你好! 我是呀哈!", 0.85); };
  document.getElementById("tts2").onclick = () => speak("你好! 我是呀哈!", 0.5);
  ...
  const ask = () => {
    if (i >= qs.length) {
      ...
      view.append(el(`<div class="card mint bigcard">
        ...
        <div class="muted">${esc(activeMascotNames()[hero.id])} ${beat ? "is so proud of you!" : "says: one more try!"}</div>
...
function writePractice() {
  ...
  const usable = chars.filter(c => window.YAHA_STROKES && window.YAHA_STROKES[c]);
  ...
  const show = () => {
    ...
    writer = HanziWriter.create("hwTarget", c, {
      ...
      charDataLoader: (ch, done) => done(window.YAHA_STROKES[ch]),
    });
...
function renderTalk() {
  ...
  if (CHATS.length) {
    ...
    CHATS.forEach(c => {
      const cast = activeCast().find(x => x.id === c.host);
      ...
      const b = el(`<button class="lesson ${locked ? "locked" : ""}">
        ...
        <br><span class="lsub">${locked ? "meet " + esc(activeMascotNames()[c.host].split(" ")[1]) + " first (" + cast.unlock + " ✨xp)" : esc(activeMascotNames()[c.host]) + " · 📍 " + esc(c.place)}</span></span>
...
function runChat(c) {
  ...
  view.append(
    el(`<div class="backrow"><button class="iconbtn" id="bk">←</button>
        ...
        <span><b>${esc(activeMascotNames()[c.host])}</b><br><span class="muted" style="font-size:.75rem">📍 ${esc(c.place)}</span></span></span>
...
/* ================= CHAOS MODE 呀哈!! (hidden easter egg) =================
   Tap/click the header mascot 5x within ~2s to unleash it: bigger confetti,
   a rapid-fire chorus of every unlocked friend's catchphrase, a shaking
   rainbow-flash banner over the mascot, and a big in-voice toast. Purely
   cosmetic — no XP, no state writes — so it can never be "gamed", and it's
   rate-limited so mashing the mascot can't spam it into a loop. */
const CHAOS_SPRITES = ["🌸", "⭐", "💮", "✨", "🎀", "🍡", "🐰", "🧋", "🎉"];
const CHAOS_LINES = ["呀哈!! CHAOS MODE!!", "呀哈呀哈呀哈!!", "乌拉乌拉!! 全开!!", "呜哇啊啊啊!! 呀哈!!"];
let chaosTapLog = [];
let chaosLastFired = 0;
const CHAOS_TAP_WINDOW = 2000;   // ms — 5 taps must land inside this window
const CHAOS_TAP_COUNT = 5;
const CHAOS_COOLDOWN = 12000;    // ms — can't re-trigger for this long

function chaosBigConfetti() {
  const box = document.getElementById("confetti");
  if (!box) return;
  for (let i = 0; i < 42; i++) {
    const c = el(`<div class="confetto">${CHAOS_SPRITES[i % CHAOS_SPRITES.length]}</div>`);
    c.style.left = Math.random() * 100 + "vw";
...
function chaosOverlayEl() {
  let ov = document.getElementById("chaosOverlay");
  if (!ov) {
    ov = el(`<div id="chaosOverlay"><div class="chaos-rainbow"></div><div class="chaos-sprites"></div>
        <div class="chaos-banner"></div></div>`);
    document.body.appendChild(ov);
  }
  return ov;
}
...
function chaosMode() {
  const now = Date.now();
  if (now - chaosLastFired < CHAOS_COOLDOWN) {
    toast("呀哈~ 还在喘气! 稍等一下再来~ (already chaotic, catch your breath!)");
    return;
  }
  chaosLastFired = now;
  chaosTapLog = [];

  // 1. the mascot itself goes feral
  const brand = document.getElementById("brandMascot");
  if (brand) {
    brand.classList.remove("chaos-shake"); void brand.offsetWidth; // restart anim if re-triggered
    brand.classList.add("chaos-shake");
    setTimeout(() => brand.classList.remove("chaos-shake"), 1600);
  }

  // 2. bigger-than-usual confetti, twice
  chaosBigConfetti();
  setTimeout(chaosBigConfetti, 350);

  // 3. rainbow-flash banner over everything (never blocks input — pointer-events:none)
  const ov = chaosOverlayEl();
  ov.querySelector(".chaos-banner").textContent = shuffle(CHAOS_LINES)[0];
  const spriteLayer = ov.querySelector(".chaos-sprites");
  spriteLayer.innerHTML = "";
  for (let i = 0; i < 24; i++) {
    const s = el(`<span class="chaos-sprite">${CHAOS_SPRITES[i % CHAOS_SPRITES.length]}</span>`);
    s.style.left = Math.random() * 100 + "%";
    s.style.fontSize = (1.3 + Math.random() * 1.6) + "rem";
    s.style.animationDelay = (Math.random() * .5) + "s";
    spriteLayer.appendChild(s);
  }
  ov.classList.add("show");
  setTimeout(() => ov.classList.remove("show"), 2000);

  // 4. big in-voice toast
  toast("呀哈!! CHAOS MODE!! 呀哈呀哈呀哈!!");

  // 5. a rapid chorus — every unlocked friend shouts one line each, back-to-back
  //    (speakAs falls back to the silent no-op path automatically if
  //    speechSynthesis / cast audio clips aren't available on this device)
  const roster = shuffle(unlockedCast()).slice(0, 5);
  roster.forEach((c, i) => {
    setTimeout(() => {
      const lines = activeCastSpeak()[c.id];
      if (lines && lines.length) speakAs(shuffle(lines)[0], c.id);
    }, i * 480);
  });
}
...
(function initChaosMode() {
  ...
  const trigger = brand || header;
  if (!trigger) return;
  trigger.addEventListener("click", () => {
    const now = Date.now();
    chaosTapLog = chaosTapLog.filter(t => now - t < CHAOS_TAP_WINDOW);
    chaosTapLog.push(now);
    if (chaosTapLog.length >= CHAOS_TAP_COUNT) chaosMode();
  });
})();
