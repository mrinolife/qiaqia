...
function mascotSVG(kind, size) {
  const s = size || 96;
  if (LOCAL_ART[kind]) return `<img src="${LOCAL_ART[kind]}" width="${s}" height="${s}" class="mascot-${kind}" style="object-fit:contain" alt="">`;
  return "";
}
...
function maybePeek() {
  ...
  const c = Math.random() < 0.62 ? CAST.find(x => x.id === "usagi") : shuffle(unlockedCast())[0];
